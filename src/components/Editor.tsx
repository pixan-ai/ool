...
export default function Editor({
  ...
  return (
    <div className="flex flex-col h-full relative">
      {/* Toolbar */}
      <div className="flex items-center gap-1 px-3 py-1.5 border-b border-[var(--border-subtle)]" style={{ height: 'var(--toolbar-height)' }}>
        {/* Back button (mobile) */}
        ...
        <div className="flex items-center gap-0.5 ml-1 pl-1 border-l border-[var(--border-subtle)]">
          ...
          <button
            ...
            className={`toolbar-btn ${isCanvas ? 'active' : ''}`}
            title="Canvas mode — write anywhere"
          >
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {/* Formatting (only in markdown mode) */}
        {!isCanvas && (
          ...
          <>
            ...
            <div className={`hidden md:block ${showPreview ? 'w-1/2' : 'w-full'} h-full overflow-hidden`}>
              <textarea
                ...
                className="editor-textarea w-full h-full p-5 md:p-8"
                spellCheck={false}
              />
            </div>
            ...
            {showPreview && (
              <div className="hidden md:block w-1/2 h-full overflow-y-auto p-5 md:p-8" style={{ background: NOTE_COLORS[note.color].bg }}>
            ...
            <div className="md:hidden w-full h-full overflow-hidden">
              {mobileTab === "write" ? (
                <textarea
                  ...
                  onKeyDown={handleKeyDown}
                  placeholder="Start writing..."
                  className="editor-textarea w-full h-full p-5"
                  spellCheck={false}
                />
              ) : (
                <div className="h-full overflow-y-auto p-5" style={{ background: NOTE_COLORS[note.color].bg }}>
                  <MarkdownPreview content={note.content} />
                </div>
              )}
            </div>
