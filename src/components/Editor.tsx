    ...
    <div className="flex flex-col h-full relative">
      ...
      <div className="flex items-center gap-0.5 px-2 sm:px-4 md:px-6 py-1.5 border-b border-[var(--border-subtle)]">
        ...
        <div className="flex items-center gap-0.5 shrink-0">
          ...
          <button onClick={() => { const r = createBlock(canvasRef, note.blocks); onBlocksChange(r.blocks); setNewBlockId(r.newId); }} className="toolbar-btn" title="Add text block">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <div ref={canvasRef} className="flex-1 overflow-y-auto relative" style={{ background: noteColor.bg }}
        onDoubleClick={(e) => {
          ...
          const id = genId();
          onBlocksChange([...note.blocks, { id, x: Math.max(2, Math.min(70, x)), y: Math.max(0, y), content: "", width: 240 }]);
          setNewBlockId(id);
        }}
      >
        ...
        <FloatingBlocks blocks={note.blocks} onChange={onBlocksChange} containerRef={canvasRef} newBlockId={newBlockId} />
